...
function optval=btm_Am_Lookback_Call(S0,r,T,sigma,q,running_minimum,N)
...
for n = N-1:-1:0
    ...
    % No need to max against immediate payoff here as 1-exp(-0*dx) = 0
    if n>= j
    ...
    % Since we are dealing with american options here, we have to
    % max(continuation value, immediate payoff) as we are able to exercise
    % the option at any time point in the option lifespan
    for k = max(j-n,1):1:j+n+1
        Wn(k+jshift) = max(exp(-r*dt)*(p*u*Wnp1(jshift+k+1)+(1-p)*d*Wnp1(jshift+k-1)), 1-exp(-k*dx));
    end
end
% Interpolation
